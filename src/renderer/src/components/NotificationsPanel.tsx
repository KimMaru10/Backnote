...
type TabKey = 'notifications' | 'overdue' | 'priority'
...
const TAB_CONFIG: Record<TabKey, { title: string; label: string; iconSize: number }> = {
  notifications: { title: 'お知らせ', label: 'お知らせ', iconSize: 18 },
  overdue: { title: '期限切れ', label: '期限切れ', iconSize: 18 },
  priority: { title: '優先度高', label: '優先度', iconSize: 18 }
}
...
export default function NotificationsPanel({
  ...
  // タブ切替時に必要なデータをフェッチ。パネルを開いた時とタブ切替時の両方で実行。
  useEffect(() => {
    if (!open) return
    let cancelled = false
    setLoading(true)
  ...
  }, [open, tab, backendUrl, onUnreadCountChange])

  // 通知タブを表示していなくても期限切れ件数（バッジ）はパネル展開時に取得しておく
  useEffect(() => {
    if (!open) return
    let cancelled = false
  ...
  return (
    <>
      ...
      <aside className="fixed right-0 top-0 z-50 h-full w-[500px] max-w-[95vw] bg-white shadow-2xl border-l border-gray-200 flex">
        {/* メインコンテンツ */}
          ...
          <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-200">
            ...
            {currentTotal > 0 && (
              <span className="text-xs font-normal text-gray-400">
                {tab === 'notifications' && unreadCount > 0 ? `${unreadCount} 件未読` : `${currentTotal} 件`}
              </span>
            )}
        ...
        <div className="w-[80px] shrink-0 bg-gray-50 border-l border-gray-200 flex flex-col py-2">
          <TabButton
            ...
            icon={<Bell size={TAB_CONFIG.notifications.iconSize} />}
          ...
          <TabButton
            ...
            icon={<AlertTriangle size={TAB_CONFIG.overdue.iconSize} />}
            active={tab === 'overdue'}
            badge={overdueCount}
          ...
          <TabButton
            ...
            icon={<Flame size={TAB_CONFIG.priority.iconSize} />}
            active={tab === 'priority'}
